...
export async function POST(req: Request) {
  ...
  const autoGenerateDemos = input.autoGenerateDemos !== false && Boolean(v0ApiKey);
  ...
  // Auto-generate v0 AI demos for qualified leads if enabled
  let demoProcessedLeads: Lead[] = contactEnrichedLeads;
  if (autoGenerateDemos && v0ApiKey) {
    demoProcessedLeads = await mapWithConcurrency(contactEnrichedLeads, 3, async (lead) => {
      if (lead.status !== 'qualified') return lead;
      try {
        const demo = await createV0Demo(lead, v0ApiKey, v0Model, input.demoQuality);
  ...
  if (leadsError && isMissingExtendedLeadColumn(leadsError.message)) {
    const legacyLeadsToSave = leadsToSave.map(
      ({ phone, address, rating, reviews_count, source_url, matched_keyword, instagram_url, demo_quality, ...legacyLead }) => legacyLead
    );
  ...
  // Stream discovered leads to BigQuery Analytics warehouse in background
...
function normalizeCampaignInput(body: Partial<CampaignInput>): CampaignInput {
  return {
    ...
    autoGenerateDemos: body.autoGenerateDemos !== false,
...
function isMissingExtendedLeadColumn(message: string) {
  return /schema cache|column/i.test(message) && /(phone|address|rating|reviews_count|source_url|matched_keyword|instagram_url)/i.test(message);
}
