...
export default function Home() {
  ...
  async function handleGenerateDemo(targetLead: Lead) {
    ...
    try {
      ...
      if (data.demoUrl) {
        ...
        setNotice({
          type: 'success',
          text: `${((data.provider || settings.demoProvider || 'agentic') as string).toUpperCase()} demo generated for ${targetLead.company_name}!`
        });
      }
    } catch (err) {
      setNotice({
  ...
  async function handleAutoGenerateAllDemos() {
    ...
    setNotice({
      type: 'info',
      text: `Auto-generating ${settings.demoProvider || 'agentic'} demos for ${pendingLeads.length} leads in background...`
    });
    ...
    setNotice({
      type: 'success',
      text: `Completed demo generation. ${successCount} new demo${successCount === 1 ? '' : 's'} built successfully.`
    });
  }
    ...
    <div className="app-shell">
      ...
      <main className="flex flex-col min-w-0 pb-8">
        ...
        <Header
          ...
          subtitle={
            activeTab === 'dashboard'
              ? 'Multi-keyword web scraping, scalable demo generation, and Email & SMS cold outreach.'
              : activeTab === 'prospecting'
              ? 'Target accounts, audited conversion gaps, and 1-click personalized pitches.'
              : activeTab === 'demos'
              ? 'Interactive demos generated by Agentic, v0, or Hybrid engines.'
              : activeTab === 'pipeline'
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <MetricCard
                ...
                badge={settings.demoProvider || 'agentic'}
                icon={Wand2}
              />
              <MetricCard
                label="Outreach Sent"
            ...
            <div className="panel p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="space-y-0.5 text-center sm:text-left">
                ...
                <p className="text-xs text-gray-500 font-medium pl-8">
                  Generate demos within budget guardrails and dispatch multi-channel Email & SMS campaigns.
                </p>
              </div>
        ...
        {activeTab === 'demos' && (
          <div className="panel p-6 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-4">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-extrabold text-gray-900">AI Demo Lab</h3>
                  <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-100">
                    {settings.demoProvider || 'agentic'} engine
                  </span>
                </div>
                <p className="text-xs text-gray-500 font-medium mt-0.5">
                  Build and preview demos with Agentic, v0, or Hybrid generation
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                >
                  {batchDemoLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                  <span>Auto-Generate Pending Demos</span>
                </button>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="mt-2 text-[11px] font-semibold text-blue-600 flex items-center justify-between">
                        <span className="flex items-center gap-1 text-gray-600">
                          ...
                          <span>{lead.demo_provider || settings.demoProvider || 'agentic'} demo</span>
                        </span>
                        {isReady && (
                ...
                {selectedDemoLead ? (
                  <div className="border border-slate-800 rounded-3xl bg-slate-950 text-white flex flex-col flex-1 overflow-hidden shadow-2xl">
                    ...
                    <div className="bg-slate-900 px-4 py-3 border-b border-slate-800 flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono text-slate-300 ml-2 truncate max-w-[220px]">
                          {selectedDemoLead.demo_url || `Demo: ${selectedDemoLead.company_name}`}
                        </span>
                      </div>
                      ...
                      {selectedDemoLead.demo_url && selectedDemoLead.demo_url.startsWith('http') ? (
                        ...
                        >
                          <iframe
                            ...
                            title={`${selectedDemoLead.company_name} Live AI Demo`}
                        ...
                        <div className="text-center space-y-4 max-w-md p-8">
                          ...
                          <h4 className="text-xl font-extrabold tracking-tight">
                            Generate Demo for {selectedDemoLead.company_name}
                          </h4>
                            ...
                            >
                              {demoGeneratingId === selectedDemoLead.id ? (
                                <>
                                  ...
                                  <span>Building AI Demo...</span>
                                </>
                              ) : (
                                <>
                                  <Sparkles className="w-4 h-4" />
                                  <span>Build Demo Now</span>
                                </>
                              )}
                            </button>
                          </div>
                    ...
                    <div className="px-4 py-2.5 bg-slate-900 border-t border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
                      <span>Engine: {selectedDemoLead.demo_provider || settings.demoProvider || 'agentic'}</span>
                  ...
                  <div className="panel p-12 text-center text-gray-500">
                    Select a lead to preview its AI demo.
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="space-y-3">
                  {pipelineColumns.demoReady.map((l) => (
                    <div key={l.id} className="p-3.5 rounded-xl bg-white border border-purple-200 shadow-sm">
                      <div className="font-bold text-xs text-gray-900">{l.company_name}</div>
                      <div className="text-[11px] text-gray-500">{l.demo_provider || 'agentic'} demo ready</div>
                      <div className="mt-2 flex items-center justify-between text-[10px]">
