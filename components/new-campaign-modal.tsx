...
import type { CampaignInput, DemoProvider, LeadSource, OutreachChannel } from '@/lib/types';
      ...
      <div className="panel bg-white max-w-xl w-full p-5 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2.5 mb-3 flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 font-medium">Scrape leads, choose a demo engine, and dispatch outreach</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto pr-1 space-y-3.5 pb-2 scrollbar-thin">
            ...
            {/* Demo Engine & Budget Guardrails */}
            <div className="p-3.5 rounded-2xl bg-gray-50/80 border border-gray-200/70">
              <div className="space-y-3">
                <div>
                  <div className="text-xs font-extrabold text-gray-900 mb-1">Demo Build Engine</div>
                  <div className="grid grid-cols-3 gap-1.5">
                    {demoProviderOptions.map((option) => (
                      <button
                        key={option.value}
                        type="button"
                        onClick={() => setForm({ ...form, demoProvider: option.value })}
                        className={`p-2 rounded-xl border text-left transition-all ${
                          form.demoProvider === option.value
                            ? 'bg-blue-50 border-blue-500 text-blue-800'
                            : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'
                        }`}
                      >
                        <div className="text-[11px] font-extrabold">{option.label}</div>
                        <div className="text-[9px] leading-tight opacity-75">{option.desc}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="text-xs font-extrabold text-gray-900">Auto-Generate Demos</div>
                    <div className="text-[10px] text-gray-500">Off by default to protect credits; applies only to strong leads</div>
                  </div>
                </div>
                <input
                  type="checkbox"
                  checked={form.autoGenerateDemos === true}
                  onChange={(e) => setForm({ ...form, autoGenerateDemos: e.target.checked })}
                  className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 cursor-pointer"
                />
                </div>
                {form.autoGenerateDemos && (
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="label text-[10px]">Max Auto Demos</label>
                      <input
                        type="number"
                        min={0}
                        max={25}
                        value={form.demoLimit || 3}
                        onChange={(e) => setForm({ ...form, demoLimit: Math.min(Math.max(Number(e.target.value) || 0, 0), 25) })}
                        className="field text-xs py-1.5"
                      />
                    </div>
                    <div>
                      <label className="label text-[10px]">Min Score</label>
                      <input
                        type="number"
                        min={0}
                        max={100}
                        value={form.demoMinScore || 75}
                        onChange={(e) => setForm({ ...form, demoMinScore: Math.min(Math.max(Number(e.target.value) || 0, 0), 100) })}
                        className="field text-xs py-1.5"
                      />
                    </div>
                  </div>
          ...
          <div className="-mx-5 -mb-5 px-5 py-3.5 bg-gray-50/90 border-t border-gray-100 flex items-center justify-end gap-3 flex-shrink-0 backdrop-blur-xs">
            ...
            >
              {loading ? 'Scraping & Scoring Leads...' : `Launch Campaign (${form.limit} Leads)`}
            </button>
          </div>
        </form>
      </div>
